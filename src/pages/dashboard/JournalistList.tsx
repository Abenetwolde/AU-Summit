...
export function JournalistList() {
    ...
    const filteredData = displayData.filter(app => {
        ...
        // Handle country filter - match both country code (e.g., "IT") and full name (e.g., "Italy")
        const matchesCountry = selectedCountry
            ? (country === selectedCountry || country === countryName(selectedCountry))
            : true;
                                    ...
                                    <tr key={app.id} className="border-b transition-colors hover:bg-muted/50">
                                        ...
                                        <td className="p-4 align-middle">
                                            {checkPermission('application:view:by-id') && (
                                                <Button variant="outline" size="sm" className="flex h-8 text-blue-500 border-blue-200 hover:bg-blue-50 hover:text-blue-700 font-bold" onClick={() => navigate(`${basePath}/journalists/${app.id}`, { state: { application: app } })}>
